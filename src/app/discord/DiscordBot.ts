...
export class DiscordBot extends Manager {
    ...
    async onUpdateActivity() {
        ...
        this.client.user.setActivity(`les ${onlineUser} Wibbo's en ligne!`, { type: "WATCHING" });
    }
    ...
    getGuildMemberFromCommuById(id: number): GuildMember {
        const user = this.client.guilds.cache.find(x => x.id == Config.discord.communGuildId).members.cache.find(x => x.id == id.toString());
